    ...
    <div className="fixed top-6 right-6 z-[999] flex flex-col gap-3 pointer-events-none">
      {toasts.map((t) => {
        ...
        return (
          <div
            key={t.id}
            className={`pointer-events-auto flex items-center gap-3 px-5 py-4 rounded-2xl border shadow-2xl text-white text-sm font-bold max-w-sm animate-in slide-in-from-right-4 duration-300 ${styles[t.type]}`}
          >
            {icons[t.type]}
  ...
  return (
    <div className="relative">
      <input
        ...
        autoComplete="off"
        placeholder="Paste provider token here..."
        value={value}
    ...
    <div className="bg-[#0A101D] border border-blue-500/20 rounded-2xl p-5 mt-4">
      ...
      <div className="flex flex-wrap gap-3 mb-4">
        <div className="flex items-center gap-2">
          ...
          <input
            type="number" min={1} max={32} value={days}
            onChange={e => setDays(Math.max(1, Math.min(32, parseInt(e.target.value) || 1)))}
...
function ApiDiagnosticPanel({ token, company, markupPercent = 10 }: { token: string; company: any; markupPercent?: number }) {
  ...
  const surcharge = Number(company?.dynamic_surcharge_percent || 0);
  const modifier = Number(company?.price_modifier || 1);
  const finalPrice = rawApiPrice != null
    ? rawApiPrice * (1 + surcharge / 100) * modifier * (1 + markupPercent / 100)
    : null;
  ...
  return (
    <div className="bg-emerald-500/5 border border-emerald-500/20 rounded-2xl p-6">
      ...
      <p className="text-[10px] text-emerald-500/70 font-bold mb-4">
        Ping the gateway with this token and see exactly what comes back.
      </p>
      <div className="flex flex-wrap gap-3 mb-4">
        <div>
          ...
          <input type="date" value={dropDate} onChange={e => setDropDate(e.target.value)}
            className="bg-[#1A2235] border border-slate-700 rounded-lg px-3 py-2 text-xs font-bold text-white outline-none focus:border-emerald-500" />
        </div>
        <div>
      ...
      {!token && (
        <p className="text-[11px] text-amber-400/80 font-bold flex items-center gap-2">
          <AlertCircle className="w-3.5 h-3.5" /> Enter a token above to enable the test.
        </p>
      )}
      ...
      {result && (
        <div className="space-y-3 mt-3">
          <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest border ${
            ...
            <span className="text-slate-500 font-bold">· {dropDate} → {pickDate}</span>
          </div>
...
function CompanyQuickStats({ company }: { company: any }) {
  const ltnAvg = company.ltn_reviews?.length
    ? (company.ltn_reviews.reduce((s: number, r: Review) => s + Number(r.rating || 0), 0) / company.ltn_reviews.length).toFixed(1)
    : null;
  const lhrAvg = company.lhr_reviews?.length
    ? (company.lhr_reviews.reduce((s: number, r: Review) => s + Number(r.rating || 0), 0) / company.lhr_reviews.length).toFixed(1)
    : null;
  ...
  const stats = [
    ...
    { label: "Badges", value: company.badges?.length || 0, sub: "active", color: "text-amber-400" },
          ...
          <div className="px-8 pb-6 pt-3 bg-[#0A101D]/60 border-t border-emerald-500/10 animate-in fade-in duration-200">
            <div className="flex flex-wrap items-end gap-3 mb-4">
              <div>
                ...
                <input
                  type="date" value={dropDate} onChange={e => setDropDate(e.target.value)}
                  className="bg-[#1A2235] border border-slate-700 rounded-lg px-3 py-2 text-xs font-bold text-white outline-none focus:border-emerald-500 [color-scheme:dark]"
                />
              </div>
              <div>
                <label className="text-[8px] font-black text-slate-500 uppercase tracking-widest block mb-1">Pick-up</label>
                <input
                  type="date" value={pickDate} min={dropDate} onChange={e => setPickDate(e.target.value)}
                  className="bg-[#1A2235] border border-slate-700 rounded-lg px-3 py-2 text-xs font-bold text-white outline-none focus:border-emerald-500 [color-scheme:dark]"
                />
              </div>
              <button
            ...
            {result && (
              <div className="space-y-3">
                <div className={`inline-flex items-center gap-2 px-3 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest border ${
                  ...
                  HTTP {result.status} · {result.ok ? "OK" : "Failed"}
                  <span className="text-slate-500 font-bold ml-1">{dropDate} → {pickDate}</span>
                </div>
                ...
                {rawApiPrice != null ? (
                  ...
                  <p className="text-[10px] text-amber-400 font-bold">No parking_price found in response. Check raw output below.</p>
                ) : null}
  ...
  if (logoUrl && !imgError) {
    return (
      <img
        src={logoUrl}
        alt={name}
        className="w-11 h-11 rounded-xl object-contain bg-white p-1.5 border border-slate-700/50 shrink-0"
...
export default function AdminCompaniesPage() {
  ...
  useEffect(() => {
    supabase.from("settings").select("*").in("key", ["markup_percent"]).then(({ data }) => {
      if (data) {
        ...
        if (pc) setMarkupPercent(Number(pc.value) || 10);
      }
    });
  }, []);
  ...
  useEffect(() => {
    if (!editingCompany || !originalEditingRef.current) {
      setHasUnsavedChanges(false);
      return;
    }
    const changed = JSON.stringify(editingCompany) !== JSON.stringify(originalEditingRef.current);
  ...
  async function fetchFinancials(companyId: string) {
    ...
    const { data, error } = await supabase
      .from("bookings").select("*").eq("company_id", companyId)
  ...
  const closeModal = () => {
    ...
    setCompanyBookings([]);
    setHasUnsavedChanges(false);
    originalEditingRef.current = null;
  };

  const openEditModal = (company: any) => {
    originalEditingRef.current = JSON.parse(JSON.stringify(company));
  ...
  const handleAddCompany = async (e: React.FormEvent) => {
    ...
    try {
      ...
      showToast("Error adding partner: " + error.message, "error");
    } finally {
      setIsSaving(false);
    }
  };
  ...
  const masterUpdate = async (val: number) => {
    const text = val === 1
      ...
      : `apply ${val > 1 ? "+" : ""}${Math.round((val - 1) * 100)}% modifier to ALL operators`;
  ...
  const quickToggle = async (company: any, field: string) => {
    ...
    if (error) {
      ...
      const labels: Record<string, string> = {
        is_active: newVal ? "Activated" : "Deactivated",
        ltn_sold_out: newVal ? "LTN marked sold out" : "LTN marked available",
        lhr_sold_out: newVal ? "LHR marked sold out" : "LHR marked available",
      };
  ...
  const updateTerminalField = (term: string, field: string, value: string) => {
    ...
    setField(editingCompany, setEditingCompany, newCompany, setNewCompany, "terminal_data", {
      ...currentData,
  ...
  const filteredAndSortedCompanies = useMemo(() => {
    ...
    if (apiFilter === "API") result = result.filter(c => !!c.api_token);
    if (apiFilter === "MANUAL") result = result.filter(c => !c.api_token);
    result.sort((a, b) => {
  ...
  const addReview = (airport: "ltn" | "lhr") => {
    ...
    const rev: Review = {
      id: Date.now(), author: "Customer Name", rating: 5,
      comment: "Write review here...",
      date: new Date().toISOString().split("T")[0], verified: true, source: "Trustpilot",
    };
  ...
  const apiCount = companies.filter(c => !!c.api_token).length;
  ...
  const inputCls = "w-full bg-[#1A2235] border border-slate-700/50 hover:border-blue-500/50 rounded-xl px-5 py-4 text-sm text-white font-bold outline-none focus:ring-2 focus:ring-blue-500/50 transition-all [&:-webkit-autofill]:[box-shadow:0_0_0_1000px_#1A2235_inset] [&:-webkit-autofill]:[-webkit-text-fill-color:white] placeholder:text-slate-500";
  const labelCls = "text-[10px] font-black uppercase text-slate-500 block ml-1 tracking-widest mb-2";
  const filterSelectCls = "w-full appearance-none bg-[#1A2235] border border-slate-700/50 hover:border-blue-500/50 rounded-xl py-4 pl-10 pr-9 text-[10px] sm:text-xs font-black uppercase tracking-widest text-slate-300 outline-none cursor-pointer transition-all [&:-webkit-autofill]:[box-shadow:0_0_0_1000px_#1A2235_inset] [&:-webkit-autofill]:[-webkit-text-fill-color:white]";
  const textareaCls = "w-full bg-[#1A2235] border border-slate-700/50 hover:border-blue-500/50 rounded-xl px-5 py-4 text-sm text-white font-bold outline-none focus:ring-2 focus:ring-blue-500/50 transition-all [&:-webkit-autofill]:[box-shadow:0_0_0_1000px_#1A2235_inset] [&:-webkit-autofill]:[-webkit-text-fill-color:white] placeholder:text-slate-500 leading-relaxed";
    ...
    <div className="min-h-screen bg-[#060A14] font-sans flex flex-col md:flex-row overflow-hidden text-slate-100 selection:bg-blue-600/30 selection:text-white antialiased">
      ...
      <aside className="w-full md:w-64 bg-[#0F1523] text-slate-400 hidden md:flex flex-col sticky top-0 h-screen border-r border-slate-800/80 shadow-2xl z-50 shrink-0">
        ...
        <nav className="px-5 space-y-3 flex-grow mt-6 font-bold text-sm">
          ...
          <Link href="/admin/companies" className="flex items-center gap-4 px-5 py-4 bg-blue-600 text-white rounded-xl shadow-[0_10px_20px_-5px_rgba(37,99,235,0.4)] hover:bg-blue-500">
          ...
          <Link href="/admin/financials" className="flex items-center gap-4 px-5 py-4 hover:bg-white/5 hover:text-white rounded-xl transition-all">
            <CalendarDays className="w-5 h-5 text-slate-500" /> Financials
          </Link>
        </nav>
        <div className="p-6">
          <button onClick={handleLogout} className="flex items-center gap-4 text-sm font-bold hover:text-red-400 transition-colors w-full text-left px-5 py-4 group bg-slate-900/50 rounded-xl border border-slate-800/80">
      ...
      <main className="flex-1 p-4 md:p-8 lg:p-12 w-full overflow-y-auto h-screen relative pb-32 md:pb-12">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-8">
          {[
            ...
            <div key={label} className={`relative bg-[#131A2B] p-6 rounded-[2rem] border border-${color}-900/30 hover:border-${color}-500/40 overflow-hidden shadow-xl transition-all cursor-default`}>
              <p className={`text-[9px] font-black uppercase tracking-[0.3em] text-${color}-400 mb-2 flex items-center gap-1.5`}>
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-6 relative z-10">
            <div>
              <h2 className="text-xl font-black text-white flex items-center gap-2">
                <Zap className="w-5 h-5 text-blue-400" /> Master Price Modifier
              </h2>
            ...
            <div className="flex flex-wrap items-center gap-2">
              {[0.7, 0.8, 0.9, 1.0, 1.1, 1.2, 1.3].map(v => (
                <button key={v} type="button" onClick={() => masterUpdate(v)}
                  className={`px-4 py-2.5 rounded-xl text-xs font-black transition-all active:scale-95 border ${v === 1
                    ? "bg-slate-800 text-white border-slate-700 hover:bg-slate-700"
                    : v < 1
                    ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20 hover:bg-emerald-500/20"
                    : "bg-rose-500/10 text-rose-400 border-rose-500/20 hover:bg-rose-500/20"}`}>
                  {v === 1 ? "BASE" : `${v > 1 ? "+" : ""}${Math.round((v - 1) * 100)}%`}
                </button>
              ))}
              <button onClick={fetchCompanies} title="Refresh"
                className="p-2.5 bg-slate-800 border border-slate-700 rounded-xl text-slate-400 hover:text-white hover:bg-slate-700 transition-all active:scale-95">
        ...
        <div className="bg-[#131A2B] rounded-[2rem] border border-slate-800 shadow-lg p-4 mb-8 flex flex-col xl:flex-row gap-4">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input
              type="text" autoComplete="off" placeholder="Search partners..." value={searchTerm}
              onChange={e => setSearchTerm(e.target.value)}
              className="w-full bg-[#1A2235] border border-slate-700/50 hover:border-blue-500/50 rounded-xl py-4 pl-12 pr-10 text-sm font-bold text-white outline-none focus:ring-2 focus:ring-blue-500/50 transition-all shadow-[0_0_0_1000px_#1A2235_inset] [-webkit-text-fill-color:white] placeholder:text-slate-500"
            />
            {searchTerm && (
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3 w-full xl:w-auto shrink-0">
            <div className="relative">
              ...
              <select value={categoryFilter} onChange={e => setCategoryFilter(e.target.value)} className={`${filterSelectCls} ${categoryFilter !== "ALL" ? "text-blue-400" : ""}`}>
            ...
            <div className="relative">
              ...
              <select value={airportFilter} onChange={e => setAirportFilter(e.target.value)} className={`${filterSelectCls} ${airportFilter !== "ALL" ? "text-blue-400" : ""}`}>
            ...
            <div className="relative">
              ...
              <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className={`${filterSelectCls} ${statusFilter !== "ALL" ? "text-blue-400" : ""}`}>
            ...
            <div className="relative">
              ...
              <select value={apiFilter} onChange={e => setApiFilter(e.target.value)} className={`${filterSelectCls} ${apiFilter !== "ALL" ? "text-amber-400" : ""}`}>
            ...
            {activeFilterCount > 0 ? (
              <button
                onClick={() => { setCategoryFilter("ALL"); setAirportFilter("ALL"); setStatusFilter("ALL"); setApiFilter("ALL"); setSearchTerm(""); }}
                className="flex items-center justify-center gap-1.5 w-full bg-[#1A2235] border border-red-500/30 text-red-400 hover:bg-red-500/10 rounded-xl py-4 text-[10px] font-black uppercase tracking-widest transition-all"
              >
                <X className="w-4 h-4" /> Clear
              </button>
            ) : (
              <div className="flex items-center justify-center gap-1.5 w-full bg-[#1A2235] border border-slate-700/50 text-slate-500 rounded-xl py-4 text-[10px] font-black uppercase tracking-widest opacity-40 cursor-not-allowed">
            ...
            <table className="w-full text-left whitespace-nowrap">
              <thead className="bg-[#0F1523] border-b border-slate-800">
                <tr className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-500">
                  <th className="px-8 py-6 cursor-pointer hover:text-white" onClick={() => toggleSort("name")}>
                    Partner Profile <SortIcon field="name" sortBy={sortBy} sortOrder={sortOrder} />
                  </th>
                  <th className="px-6 py-6 text-center">Status</th>
                  <th className="px-6 py-6 text-center">Quick Actions</th>
                  <th className="px-6 py-6 text-center">Modifier</th>
                  <th className="px-6 py-6 text-right cursor-pointer hover:text-white" onClick={() => toggleSort("luton_price")}>
                    LTN <SortIcon field="luton_price" sortBy={sortBy} sortOrder={sortOrder} />
                  </th>
                  <th className="px-6 py-6 text-right cursor-pointer hover:text-white" onClick={() => toggleSort("heathrow_price")}>
                    LHR <SortIcon field="heathrow_price" sortBy={sortBy} sortOrder={sortOrder} />
                  </th>
              ...
              <tbody className="divide-y divide-slate-800/60">
                {filteredAndSortedCompanies.length === 0 ? (
                  ...
                  // ── FIX: React.Fragment with key so both rows are properly keyed ──
                  <React.Fragment key={c.id}>
                    ...
                    <tr className={`transition-all group hover:bg-slate-800/30 ${!c.is_active ? "opacity-50 grayscale" : ""}`}>
                      ...
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-4">
                          {/* ── FIX: CompanyLogo handles broken images gracefully ── */}
                          ...
                          <div>
                            ...
                            <div className="flex items-center gap-2 mt-1 flex-wrap">
                              ...
                              {c.api_token && (
                                <span className="text-[9px] font-black text-emerald-400 tracking-widest uppercase bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/20 flex items-center gap-1">
                                  <Zap className="w-2.5 h-2.5" /> LIVE API
                                </span>
                              )}
                            </div>
                            <div className="flex items-center gap-3 text-[10px] font-bold text-slate-400 mt-1.5">
                              {c.operates_at_luton && (
                                <span className={c.ltn_featured ? "text-amber-400" : ""}>
                                  <Car className="w-3 h-3 inline mr-0.5" /> {getAvgRating(c.ltn_reviews)}
                                </span>
                              )}
                              {c.operates_at_heathrow && (
                                <span className={c.lhr_featured ? "text-amber-400" : ""}>
                                  <PlaneTakeoff className="w-3 h-3 inline mr-0.5" /> {getAvgRating(c.lhr_reviews)}
                                </span>
                              )}
                            </div>
                          </div>
                      ...
                      <td className="px-6 py-6 text-center">
                        <button
                          onClick={() => quickToggle(c, "is_active")}
                          className={`px-3 py-1.5 rounded-lg text-[9px] tracking-widest font-black border uppercase transition-all ${c.is_active
                            ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/20 hover:bg-emerald-500/20"
                      ...
                      <td className="px-6 py-6 text-center">
                        <div className="flex items-center justify-center gap-1.5">
                          <button
                            title={c.ltn_sold_out ? "Mark LTN Available" : "Mark LTN Sold Out"}
                            onClick={() => quickToggle(c, "ltn_sold_out")}
                            className={`px-2 py-1.5 rounded-lg text-[8px] font-black border uppercase transition-all ${c.ltn_sold_out
                              ? "bg-red-500/20 text-red-400 border-red-500/30"
                              : "bg-slate-800 text-slate-500 border-slate-700 hover:border-red-500/30 hover:text-red-400"}`}
                          >
                            {c.ltn_sold_out ? "LTN 🔴" : "LTN ✓"}
                          </button>
                          <button
                            title={c.lhr_sold_out ? "Mark LHR Available" : "Mark LHR Sold Out"}
                            onClick={() => quickToggle(c, "lhr_sold_out")}
                            className={`px-2 py-1.5 rounded-lg text-[8px] font-black border uppercase transition-all ${c.lhr_sold_out
                              ? "bg-red-500/20 text-red-400 border-red-500/30"
                              : "bg-slate-800 text-slate-500 border-slate-700 hover:border-red-500/30 hover:text-red-400"}`}
                          >
                            {c.lhr_sold_out ? "LHR 🔴" : "LHR ✓"}
                          </button>
                        </div>
                      </td>
                      ...
                      {/* Modifier */}
                      <td className="px-6 py-6 text-center">
                        <div className={`inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-black border ${(c.price_modifier || 1) === 1
                          ...
                          {(c.price_modifier || 1) === 1
                            ? "BASE"
                            : `${c.price_modifier > 1 ? "+" : ""}${Math.round(((c.price_modifier || 1) - 1) * 100)}%`}
                        </div>
                      </td>
                        ...
                        {c.operates_at_luton
                          ? <span className="font-black text-white text-base tracking-tighter">
                              {c.api_token
                                ...
                                : `£${Number(c.luton_price || 0).toFixed(2)}`}
                            </span>
                      ...
                      <td className="px-6 py-6 text-right">
                        {c.operates_at_heathrow
                          ? <span className="font-black text-white text-base tracking-tighter">
                              £{Number(c.heathrow_price || 0).toFixed(2)}
                            </span>
                          : <span className="text-slate-600 text-xs">—</span>}
                      </td>
                      ...
                      <td className="px-6 py-6 text-right">
                        <div className="flex items-center justify-end gap-1.5 opacity-30 group-hover:opacity-100 transition-all">
                          <button
                            ...
                            className="p-2.5 bg-[#1A2235] text-slate-300 hover:bg-blue-600 hover:text-white rounded-lg border border-slate-700 hover:border-transparent transition-all active:scale-95"
                            title="Configure"
                          >
                            <Settings2 className="w-4 h-4" />
                          </button>
                          <button
                            onClick={() => duplicateCompany(c)}
                            className="p-2.5 bg-[#1A2235] text-slate-500 hover:bg-indigo-600 hover:text-white rounded-lg border border-slate-700 hover:border-transparent transition-all active:scale-95"
                            title="Duplicate"
                          >
                          ...
                          <button
                            ...
                            className="p-2.5 bg-[#1A2235] text-slate-500 hover:bg-red-500 hover:text-white rounded-lg border border-slate-700 hover:border-transparent transition-all active:scale-95"
                            title="Delete"
                          >
                    ...
                    {/* API Rate Checker row — only for companies with a token */}
                    {c.api_token && (
                      <tr className="bg-[#0A101D]/40">
                        <RowApiChecker company={c} markupPercent={markupPercent} />
                      </tr>
                    )}
                  </React.Fragment>
                ))}
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-[100] px-4 pb-6 pt-2 bg-gradient-to-t from-[#0B1120] via-[#0B1120]/95 to-transparent pointer-events-none">
        <nav className="max-w-md mx-auto bg-slate-900/95 backdrop-blur-3xl border border-white/10 rounded-[2.5rem] h-20 flex items-center justify-around px-5 shadow-2xl pointer-events-auto">
          ...
          <Link href="/admin/companies" className="flex flex-col items-center gap-1 text-blue-500 scale-110">
            <Building2 className="w-6 h-6" />
            <span className="text-[9px] font-bold uppercase">Ops</span>
          </Link>
          <div className="relative -top-8">
            <button
              onClick={() => { setModalTab("general"); setShowAddModal(true); }}
              className="w-16 h-16 bg-blue-600 rounded-2xl flex items-center justify-center shadow-lg shadow-blue-500/30 border-4 border-[#0B1120] active:scale-95 transition-transform"
            >
              <Plus className="w-8 h-8 text-white" />
            </button>
          </div>
          <Link href="/admin/financials" className="flex flex-col items-center gap-1 text-slate-500 hover:text-slate-300 transition-colors">
            <PiggyBank className="w-6 h-6" />
            <span className="text-[9px] font-bold uppercase">Money</span>
          </Link>
              ...
              <div className="flex justify-between items-start mb-6">
                <div>
                  <div className="flex items-center gap-3">
                    ...
                    {hasUnsavedChanges && (
                      <span className="flex items-center gap-1.5 px-2.5 py-1 bg-amber-500/10 border border-amber-500/30 rounded-lg text-[9px] font-black uppercase tracking-widest text-amber-400">
                  ...
                  {editingCompany && (
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1 flex items-center gap-1.5">
                      ...
                      <CopyButton text={editingCompany.id} />
                    </p>
                  )}
                </div>
                <button
                  onClick={closeModal}
                  className="p-3 bg-[#1A2235] rounded-xl text-slate-400 hover:text-white hover:bg-red-500/20 transition-colors border border-slate-700/50"
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
              ...
              <div className="flex gap-8 overflow-x-auto pb-px">
                {[
                  ...
                  <button
                    ...
                    className={`pb-4 text-[10px] font-black uppercase tracking-widest border-b-2 transition-colors whitespace-nowrap flex items-center gap-2 ${modalTab === key
                      ? key === "financials" ? "border-emerald-500 text-emerald-400" : "border-blue-500 text-blue-400"
                  ...
                  <div className="flex-1 overflow-y-auto p-8">
                      ...
                      <div className="space-y-8 text-white">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          ...
                          <div className="space-y-2">
                            <label className={labelCls}>
                              <Network className="w-3.5 h-3.5 inline mr-1 text-emerald-500" /> Live API Token (Optional)
                            </label>
                            <TokenInput
                              value={getField(editingCompany, newCompany, "api_token") || ""}
                              onChange={v => setField(editingCompany, setEditingCompany, newCompany, setNewCompany, "api_token", v)}
                              inputCls={inputCls}
                            />
                            <p className="text-[10px] text-slate-500 font-bold">If set, prices are fetched live from the provider gateway. Manual pivots act as fallback.</p>
                          </div>
                        </div>
                        ...
                        <div className="bg-[#1A2235] p-6 rounded-2xl border border-slate-700/50">
                          <h3 className="text-sm font-black text-white mb-2 flex items-center gap-2">
                            <Zap className="w-4 h-4 text-amber-400" /> Custom Price Modifier
                          </h3>
                          <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest mb-4">
                            Per-operator multiplier applied on top of any price source.
                          </p>
                          <div className="flex flex-wrap gap-2">
                            {[0.7, 0.8, 0.9, 1.0, 1.1, 1.2, 1.3].map(v => (
                              <button
                                ...
                                className={`px-4 py-2 rounded-xl text-xs font-black transition-all border ${(getField(editingCompany, newCompany, "price_modifier") || 1.0) === v
                        ...
                        {/* API Diagnostic — only when token set */}
                        {getField(editingCompany, newCompany, "api_token") && (
                          <ApiDiagnosticPanel
                            token={getField(editingCompany, newCompany, "api_token") || ""}
                            company={editingCompany || newCompany}
                            markupPercent={markupPercent}
                          />
                        )}

                        {/* Price Simulator */}
                        <PricePreviewCalc company={editingCompany || newCompany} />
                        ...
                        <div className="bg-[#1A2235] p-6 rounded-2xl border border-slate-700/50">
                          ...
                          <div className="flex gap-2 mb-4">
                            ...
                            <select id="new-badge-cat" className="bg-[#0F1523] border border-slate-700 text-white rounded-xl px-4 text-xs font-bold outline-none shrink-0">
                            ...
                            <button
                              ...
                              className="bg-blue-600 px-6 rounded-xl text-white font-black hover:bg-blue-500 transition-colors shrink-0"
                            >
                              +
                            </button>
                          </div>
                          <div className="flex flex-wrap gap-2">
                            {(getField(editingCompany, newCompany, "badges") || []).map((b: any, i: number) => (
                              <div key={i} className="flex items-center gap-2 bg-blue-600 px-3 py-1.5 rounded-lg text-[10px] font-bold text-white uppercase">
                                {b.label}
                                <span className="opacity-50 text-[8px]">({b.category})</span>
                                <button type="button" onClick={() => handleRemoveBadge(i)} className="hover:text-red-300 ml-1">×</button>
                              </div>
                            ))}
                          </div>
                        </div>

                        {/* Category, Logo, Commission, Surcharge */}
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div className="space-y-2">
                          ...
                          <div className="space-y-2">
                            <label className={labelCls}>
                              <ImageIcon className="w-3 h-3 inline mr-1 text-blue-500" /> Logo URL
                            </label>
                            <input
                            ...
                            />
                            {/* Logo preview */}
                            {getField(editingCompany, newCompany, "logo_url") && (
                              <div className="mt-2 flex items-center gap-3">
                                <CompanyLogo
                                  logoUrl={getField(editingCompany, newCompany, "logo_url")}
                                  name={getField(editingCompany, newCompany, "name") || "?"}
                                />
                                <p className="text-[10px] text-slate-500 font-bold">Logo preview</p>
                              </div>
                            )}
                          </div>
                          <div className="space-y-2">
                            <label className={labelCls}>
                              <Percent className="w-3 h-3 inline mr-1 text-emerald-500" /> Commission Cut (%)
                            </label>
                            <input
                          ...
                          <div className="space-y-2">
                            <label className={labelCls}>
                              <Zap className="w-3 h-3 inline mr-1 text-amber-500" /> Dynamic Surcharge (%)
                            </label>
                            <input
                              type="number" step="0.5" min="0"
                              value={getField(editingCompany, newCompany, "dynamic_surcharge_percent") || 0}
                              onChange={e => setField(editingCompany, setEditingCompany, newCompany, setNewCompany, "dynamic_surcharge_percent", parseFloat(e.target.value) || 0)}
                              className={`${inputCls} text-amber-400 !text-xl`}
                            />
                            <p className="text-[10px] text-slate-500 font-bold mt-1 leading-relaxed">
                              Adds a % on top of the API base price to separate providers with same token. e.g. APD = 4% above base.
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-slate-800/80">
                          <div className="space-y-2">
                            <label className={labelCls}>
                              <Phone className="w-3.5 h-3.5 inline mr-1 text-amber-500" /> Dispatch Phone 1
                            </label>
                            <input
                              type="text" autoComplete="off" placeholder="07700..."
                          ...
                          <div className="space-y-2">
                            <label className={labelCls}>
                              <Phone className="w-3.5 h-3.5 inline mr-1 text-amber-500" /> Dispatch Phone 2 (Optional)
                            </label>
                            <input
                              type="text" autoComplete="off" placeholder="07700..."
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-4 border-t border-slate-800/80">
                          <div className="space-y-2 md:col-span-2">
                            <label className={labelCls}>
                              <MapPin className="w-3.5 h-3.5 inline mr-1 text-emerald-500" /> Physical Address
                            </label>
                            <input
                        ...
                        <div className="space-y-2">
                          <label className={labelCls}>
                            <MapPin className="w-3.5 h-3.5 inline mr-1 text-emerald-500" /> Google Map Embed URL
                          </label>
                          <input
                            type="text" autoComplete="off" placeholder="http://..."
                        ...
                        <div className="space-y-2 pt-4 border-t border-slate-800">
                          <label className={labelCls}>
                            <Code2 className="w-3.5 h-3.5 inline mr-1 text-blue-500" /> Marketing Overview (HTML Support)
                          </label>
                          <textarea
                            rows={4}
                          ...
                          <p className="text-[10px] text-blue-500 font-bold uppercase tracking-widest">
                            Use &lt;br/&gt; for line breaks, &lt;b&gt;text&lt;/b&gt; for bold.
                          </p>
                        </div>
                      </div>
                    )}
                    ...
                    {modalTab === "terminals" && (
                      <div className="space-y-6 text-white">
                        <div className="bg-[#131A2B] p-6 rounded-2xl border border-slate-800 mb-4">
                          <h3 className="text-lg font-black text-white mb-2">Heathrow Terminal Maps & Addresses</h3>
                          <p className="text-xs text-slate-400">Injected into customer confirmation emails based on terminal selected at checkout.</p>
                        </div>
                            ...
                            <div key={term} className="bg-[#131A2B] p-6 rounded-2xl border border-slate-800">
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div className="space-y-2">
                                  ...
                                  <input type="text" className={inputCls} value={tData?.address || ""} onChange={e => updateTerminalField(term, "address", e.target.value)} />
                                </div>
                                <div className="space-y-2">
                                  <label className={labelCls}>Postcode</label>
                                  <input type="text" className={inputCls} value={tData?.postcode || ""} onChange={e => updateTerminalField(term, "postcode", e.target.value)} />
                                </div>
                                <div className="space-y-2 md:col-span-2">
                                  <label className={labelCls}>Google Maps Link (For Emails)</label>
                                  <input type="text" className={inputCls} value={tData?.map_url || ""} onChange={e => updateTerminalField(term, "map_url", e.target.value)} />
                                </div>
                              </div>
                            </div>
                          );
                        ...
                        {getField(editingCompany, newCompany, "operates_at_luton") && (
                          <>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 border-b border-slate-800 pb-8">
                              <div className="space-y-2">
                                <label className={labelCls}>Day 1: Starting Price (£)</label>
                                <input
                              ...
                              <div className="flex flex-col gap-3 pt-6">
                                <button
                                  ...
                                  className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] border transition-all flex items-center justify-center gap-2 ${getField(editingCompany, newCompany, "ltn_sold_out")
                                    ? "bg-red-500/20 text-red-400 border-red-500/30"
                                    : "bg-[#1A2235] text-slate-400 border-slate-700/50 hover:border-slate-600"}`}
                                >
                                ...
                                <button
                                  ...
                                  className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] border transition-all flex items-center justify-center gap-2 ${getField(editingCompany, newCompany, "ltn_featured")
                            ...
                            <div className="pt-2">
                              <p className="text-sm font-black text-white mb-1">Pivot Points (Manual Pricing Fallback)</p>
                              <p className="text-[10px] text-slate-500 uppercase tracking-widest font-bold mb-5">
                                Exact TOTAL PRICE at specific durations. Engine interpolates between them.
                              </p>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-8 border-t border-slate-800">
                              <div className="space-y-2">
                                <label className={labelCls}>Arrival Instructions (HTML)</label>
                              ...
                              <div className="space-y-2">
                                <label className={labelCls}>Return Instructions (HTML)</label>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 border-b border-slate-800 pb-8">
                              ...
                              <div className="flex flex-col gap-3 pt-6">
                                <button
                                  ...
                                  className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] border transition-all flex items-center justify-center gap-2 ${getField(editingCompany, newCompany, "lhr_sold_out")
                                    ? "bg-red-500/20 text-red-400 border-red-500/30"
                                    : "bg-[#1A2235] text-slate-400 border-slate-700/50 hover:border-slate-600"}`}
                                >
                                  <AlertOctagon className="w-3.5 h-3.5" /> Mark Sold Out
                                ...
                                <button
                                  ...
                                  className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] border transition-all flex items-center justify-center gap-2 ${getField(editingCompany, newCompany, "lhr_featured")
                                    ? "bg-amber-500/20 text-amber-400 border-amber-500/30"
                                    : "bg-[#1A2235] text-slate-400 border-slate-700/50 hover:border-slate-600"}`}
                                >
                                  <Award className="w-3.5 h-3.5" /> Featured Provider
                  ...
                  <div className="bg-[#131A2B] p-8 shrink-0 border-t border-slate-800">
                    <div className="flex gap-3">
                      <button
                        ...
                        className="px-6 py-4 bg-slate-800 hover:bg-slate-700 text-slate-300 rounded-xl font-black uppercase tracking-[0.2em] text-xs transition-all border border-slate-700"
                      >
                        Cancel
                      </button>
                      <button
                        type="submit" disabled={isSaving}
                        className="flex-1 bg-blue-600 hover:bg-blue-500 py-4 rounded-xl font-black uppercase tracking-[0.2em] text-xs text-white shadow-[0_10px_20px_-5px_rgba(37,99,235,0.4)] transition-all active:scale-95 disabled:opacity-50 flex items-center justify-center gap-3"
                      >
                        {isSaving
                          ? <><Loader2 className="animate-spin w-4 h-4" /> Syncing Network...</>
                          : <><Save className="w-4 h-4" /> Save &amp; Deploy Partner</>}
                      </button>
                    </div>
                  </div>
                </form>
                    ...
                    <>
                      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-6 bg-emerald-500/10 border border-emerald-500/20 p-8 rounded-3xl">
                        ...
                        <button
                          ...
                          className="w-full sm:w-auto px-6 py-4 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-[10px] font-black uppercase tracking-[0.2em] transition-all flex items-center justify-center gap-2"
                        >
                      ...
                      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
                        {[
                          { label: "Total Gross Collected", value: `£${calcGross().toFixed(2)}`, color: "text-white" },
                          ...
                          <table className="w-full text-left whitespace-nowrap">
                            ...
                            <tbody className="divide-y divide-slate-800/50">
                                ...
                                return (
                                  <tr key={b.id} className="hover:bg-white/[0.02]">
                                    <td className="px-8 py-4">
                                      <span className="text-xs font-bold text-white">{b.booking_ref}</span>
                                      <CopyButton text={b.booking_ref} />
                                    </td>
                                    <td className="px-8 py-4 text-[10px] font-black uppercase text-slate-500 tracking-widest">
                                      {b.service_type || "Meet & Greet"}
                                    </td>
                              ...
                              {companyBookings.length === 0 && (
                                <tr>
                                  <td colSpan={5} className="px-8 py-10 text-center text-slate-500 text-xs font-bold">
...
function ReviewSection({ airport, color, reviews, onAdd, onRemove, onUpdate }: {
  ...
  const inputCls = `w-full bg-[#1A2235] rounded-xl px-4 py-3 text-sm font-bold text-white border border-slate-700/50 outline-none focus:ring-2 ${ring} transition-all [&:-webkit-autofill]:[box-shadow:0_0_0_1000px_#1A2235_inset] [&:-webkit-autofill]:[-webkit-text-fill-color:white] placeholder:text-slate-500`;
    ...
    <div className="pt-8 border-t border-slate-800 mt-6">
      <div className="flex justify-between items-center mb-6">
        ...
        <button
          ...
          className={`px-5 py-3 ${accent} text-white rounded-xl text-[9px] font-black uppercase tracking-[0.2em] transition-all shadow-lg`}
        >
          + Add Review
        </button>
      </div>
        ...
        {reviews.map((rev, idx) => (
          <div key={rev.id} className="bg-[#0F1523] p-6 rounded-2xl border border-slate-700/50 relative">
            <button
              ...
              className="absolute top-6 right-6 p-2 text-slate-600 hover:text-red-500 hover:bg-red-500/10 rounded-lg transition-colors"
            >
              <Trash2 className="w-4 h-4" />
            </button>
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-4 pr-10">
              <div className="space-y-1">
                <label className="text-[9px] font-black uppercase text-slate-500 tracking-widest block ml-1">Author</label>
                <input value={rev.author || ""} onChange={e => onUpdate(idx, "author", e.target.value)} className={inputCls} />
              </div>
              <div className="space-y-1">
                <label className="text-[9px] font-black uppercase text-slate-500 tracking-widest block ml-1">Rating</label>
                <div className="relative">
                  <select
                    value={rev.rating || 5}
                    onChange={e => onUpdate(idx, "rating", parseInt(e.target.value) || 5)}
                    className={`${inputCls} appearance-none cursor-pointer text-amber-400`}
                  >
              ...
              <div className="space-y-1">
                <label className="text-[9px] font-black uppercase text-slate-500 tracking-widest block ml-1">Source</label>
                <div className="relative">
                  <select
                    value={rev.source || "Trustpilot"}
              ...
              <div className="space-y-1">
                <label className="text-[9px] font-black uppercase text-slate-500 tracking-widest block ml-1">Date</label>
                <input type="date" value={rev.date || ""} onChange={e => onUpdate(idx, "date", e.target.value)} className={`${inputCls} cursor-pointer`} />
              </div>
            </div>
            <div className="space-y-1 mb-4">
              <label className="text-[9px] font-black uppercase text-slate-500 tracking-widest block ml-1">Comment</label>
              <textarea value={rev.comment || ""} onChange={e => onUpdate(idx, "comment", e.target.value)} className={`${inputCls} resize-none leading-relaxed`} rows={3} />
            </div>
            <label className="flex items-center gap-2 cursor-pointer w-fit group">
              <input
                type="checkbox" checked={!!rev.verified}
                onChange={e => onUpdate(idx, "verified", e.target.checked)}
                className="w-4 h-4 rounded border-slate-700 bg-[#1A2235] accent-emerald-500 cursor-pointer"
              />
