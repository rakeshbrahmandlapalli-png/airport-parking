...
// 🟢 BYPASS EVERYTHING: Give the mailer its own direct database connection
...
export const sendBookingReceipt = async (booking: any, passedCompany: any, isAmendment: boolean = false) => {
  try {
    ...
    // 🟢 THE ULTIMATE FAILSAFE: If the company got lost anywhere in the code, the email fixes it itself!
    if (!company) {
      console.log("⚠️ Mailer received an empty company object. Attempting self-repair...");
      try {
        if (booking.company_id && booking.company_id !== "null" && booking.company_id !== "ALL") {
          const { data } = await supabase.from('companies').select('*').eq('id', booking.company_id).maybeSingle();
          if (data) company = data;
        }
        // If it's STILL empty, hard-lock it to Airport Parking Bay for your tests!
        if (!company) {
    ...
    const arrivalInstructions = isLuton 
      ? (company?.on_arrival_ltn || company?.luton_arrival || company?.on_arrival || 'Please call the driver 20 minutes before arrival to confirm your exact arrival time.') 
      : (company?.on_arrival_lhr || company?.heathrow_arrival || company?.on_arrival || 'Please call the driver 20 minutes before arrival to confirm your exact arrival time.');
    
    const returnInstructions = isLuton 
      ? (company?.on_return_ltn || company?.luton_return || company?.on_return || 'Instructions for vehicle collection will be provided at the terminal or via phone.') 
      : (company?.on_return_lhr || company?.heathrow_return || company?.on_return || 'Instructions for vehicle collection will be provided at the terminal or via phone.');
    ...
    // 🟢 FIXED: Scans specifically for "dispatch_phone_1" to match your Admin Panel Database!
    const phone1 = company?.dispatch_phone_1 || company?.dispatch_phone || company?.phone_number || '07700 900 123';
    const phone2 = company?.dispatch_phone_2 || company?.dispatch_phone2 || company?.phone_number_2 || '';
          ...
          <div style="background-color: #ffffff; padding: 40px 30px; border-radius: 20px; box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.05); text-align: center;">
            ...
            <div style="margin: 40px 0;">
              <a href="https://uk.trustpilot.com/evaluate/aeroparkdirect.co.uk" 
                 style="background-color: #059669; color: white; padding: 18px 32px; text-decoration: none; border-radius: 16px; font-weight: 900; font-size: 14px; text-transform: uppercase; letter-spacing: 1px; display: inline-block;">
