  ...
  if (event.type === "checkout.session.completed") {
    ...
    try {
      if (m?.is_amendment === "true") {
        ...
        // 🟢 FIXED 1: Removed the length > 10 limit!
        // 🟢 FIXED 2: Bypassed Prisma to use Supabase directly!
        ...
        // Attempt 2: Safely search by exact name without crashing
        ...
        if (!resolvedCompany) {
           const { data } = await supabase.from('companies').select('*').ilike('name', '%AeroPark Direct%').limit(1).maybeSingle();
